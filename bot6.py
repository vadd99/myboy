...
class ThreeXUIApi:
    ...
    async def add_client(self, inbound_id: int, settings: dict, total_gb: int, expire_days: int, enable: bool = True):
        ...
        payload = {
            ...
            "settings": json.dumps({"clients": [settings_for_payload]})
        }
        ...
        raise Exception(f"Gagal menambahkan klien: {error_msg}")
...
async def create_new_user(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        client_settings = {
            ...
            "alterId": 0,
            "limitIp": 0,
            "totalGB": limit_gb * 1024 * 1024 * 1024 if limit_gb else 0,
            "expiryTime": int((datetime.now() + timedelta(days=expire_days)).timestamp() * 1000) if expire_days > 0 else 0,
            "enable": True,
            "tgId": "",
            "subId": "",
            "reset": 0
